...
export default async function PatientsPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <div>
      <h1>환자 관리</h1>
      <form><input className="input" name="q" placeholder="이름/차트번호 검색" defaultValue={q} /></form>
      <Link className="btn" href="/admin/patients/new">환자 추가</Link>
      <table><thead><tr><th>이름</th><th>차트번호</th><th>생년월일</th><th>최근 상태</th><th>생성일</th></tr></thead>
      <tbody>{rows.map((p) => {
        const cases = db.cases.filter((c) => c.patientId === p.id);
        const latest = cases.at(-1)?.status || '-';
        return <tr key={p.id}><td><Link href={`/admin/patients/${p.id}`}>{p.fullName}</Link></td><td>{p.chartNo}</td><td>{p.birthDate}</td><td>{latest}</td><td>{p.createdAt.slice(0,10)}</td></tr>;
      })}</tbody></table>
    </div>
  );
}
